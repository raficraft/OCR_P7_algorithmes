class ListingEvent {
  constructor(options) {
    ...
    this.rootElement = document.querySelector(
      `#inputTrigger--${options.context}`
    ); //Label
    this.input = document.querySelector(
      `#inputTrigger--${options.context} [data-js="search"]`
    ); //Input
    this.listing = document.querySelector(`#inputList--${options.context}`); //Listing
    this.tagsLink = document.querySelectorAll(
      `#inputList--${options.context} [data-js="getTag"]`
    );

    this.arrowOpen = document.querySelector(
      `#inputTrigger--${options.context} [data-js="openListing"]`
    );
    this.arrowClose = document.querySelector(
      `#inputTrigger--${options.context} [data-js="closeListing"]`
    );
  ...
  openListing() {
    ...
    this.input.setAttribute(
      "placeholder",
      `Rechercher un ${this.options.context}`
    );
  ...
  getSuggestion() {
    ...
    if (dataSuggestion) {
      ...
      if (
        this.rootElement.dataset.status === "close" ||
        this.rootElement.dataset.status === "openSuggestion"
      ) {
        this.rootElement.setAttribute("data-status", "openSuggestion");
        const listingComponent = makeComponent(
          new createListing(sortData, this.options)
        );
